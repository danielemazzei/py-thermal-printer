...
from community.floyd.thermalprinter import thermalprinter

# create a printer passing the serial port name. Printer RX wire have to be connected 
...
# another serial port for printing to the console
s= streams.serial(SERIAL0)


# the code symbology and the message containing the digits to be printed as barcode have to be passed as strings while the number of digits as int.
# In this example the number of digits is passed as numbers withouth using len(message) in order to clearly specify the max number of allowed digits.
...
while True:
    ...
    for i in range(len(codes)):   
        ...
        # prepare a string to be written as text before the barcode and also to the serial port connected to the Viper console 
        string="Printing:"+str(msgs[i])+" with code:"+str(codes[i])+"\n" 
        print(string)
        print("-"*20)
        
        # print the string as text to the printer
        p.print_text(string)
